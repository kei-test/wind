...
public class RollingService {
    ...
    public static final List<Integer> VALID_PRD_IDS = IntStream.rangeClosed(200, 299)
            .boxed()
     ...
     */
    public RollingResponseDTO applyForRolling(PrincipalDetails principalDetails, HttpServletRequest request) {
        ...
        boolean alreadyApplied = rollingTransactionRepository.existsByUserAndCreatedAtGreaterThanEqual(user, today.atStartOfDay());
        if (alreadyApplied) {
        ...
        if (totalBetAmountFromYesterday.compareTo(MINIMUM_BET_AMOUNT) < 0) {
            throw new RestControllerException(ExceptionCode.BET_AMOUNT_TOO_LOW, "어제 슬롯 게임에서 배팅한 총 금액이 5만원 미만이면 신청 불가합니다.");
        }
        if (user.getLv() == 8 || user.getLv() == 9) {
            throw new RestControllerException(ExceptionCode.USER_LEVEL_NOT_ALLOWED, "8,9 레벨은 신청 불가합니다.");
        }

        Optional<RollingRewardRate> rewardRateOpt = rollingRewardRateRepository.findById(user.getLv());
        ...
        RollingTransaction savedTransaction = createAndApproveRollingTransaction(user, totalBetAmountFromYesterday, calculatedReward, totalBalanceRechargedYesterday, rewardPercentage, clientIp);
     ...
     */
    private RollingTransaction createAndApproveRollingTransaction(User user, BigDecimal betAmount, BigDecimal calculatedReward, BigDecimal lastDayChargeBalance, BigDecimal rate, String clientIp) {
        RollingTransaction rollingTransaction = new RollingTransaction();
        rollingTransaction.setUser(user);
        rollingTransaction.setLv(user.getLv());
        rollingTransaction.setLastDayAmount(betAmount);
        rollingTransaction.setCalculatedReward(calculatedReward);
        rollingTransaction.setRate(rate);
        rollingTransaction.setLastDayChargeSportsBalance(lastDayChargeBalance);
        rollingTransaction.setStatus(RollingTransactionEnum.APPROVAL);
        rollingTransaction.setCasinoBalance(user.getWallet().getCasinoBalance());
        rollingTransaction.setSportsBalance(user.getWallet().getSportsBalance());
        rollingTransaction.setUserIp(clientIp);
        rollingTransaction.setUsername(user.getUsername());
        rollingTransaction.setNickname(user.getNickname());
        rollingTransaction.setCreatedAt(LocalDateTime.now(ZoneId.of("Asia/Seoul")));
        rollingTransaction.setProcessedAt(LocalDateTime.now(ZoneId.of("Asia/Seoul")));

        return rollingTransactionRepository.save(rollingTransaction);
    }
